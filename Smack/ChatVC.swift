...
class ChatVC: UIViewController, UITableViewDelegate, UITableViewDataSource  {
    ...
    @IBOutlet weak var channelDescLbl: UILabel!
    ...
    override func viewDidLoad() {
        ...
        messageTextBox.layer.sublayerTransform = CATransform3DMakeTranslation(5, 0, 0)
...
//        }
    ...
    @IBAction func messageSendTapped(_ sender: UIButton) {
        
        if AuthService.instance.isLoggedIn {
            guard let channelId = MessageService.instance.selectedChannel?.id else { return }
            guard let message = messageTextBox.text else { return }
            
            SocketService.instance.addMessage(messageBody: message, userId: UserDataService.instance.id, channelId: channelId, completion: { (sucess) in
            ...
            MessageService.instance.findAllChannel { (sucess) in
                if sucess{
                    if MessageService.instance.channels.count > 0{
                        ...
                        self.channelNameLbl.text = "No messages yet"
                    }
                }
            }
        } else {
    ...
    func updateWithChannel(){
        ...
        let channelDescription = MessageService.instance.selectedChannel?.channelDescription ?? ""
        channelNameLbl.text = "#\(channelName)"
        channelDescLbl.text = channelDescription
        getMessages()
    }
